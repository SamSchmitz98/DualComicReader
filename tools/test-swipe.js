...
async function drag({ from, to, target = surface, readerTurns = 0, ms = 0 }) {
  clicked = [];
...
(async () => {
  page = 96;
  await drag({ from: [900, 400], to: [600, 410], readerTurns: +1 });
  ...
  page = 96;
  await drag({ from: [900, 400], to: [870, 400] });
  check('a 30px drag is a click, not a swipe', page === 96 && clicked.length === 0);

  await drag({ from: [900, 300], to: [800, 600] });
  ...
  state.enabled = false;
  await drag({ from: [900, 400], to: [600, 400] });
